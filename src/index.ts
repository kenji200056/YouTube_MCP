  ...
  async getTranscript(videoId: string, lang: string): Promise<string> {
    try {
      ...
      throw new McpError(
        ...
        `字幕の取得に失敗しました: ${(error as Error).message}`
      );
    }
  }
    ...
    switch (name) {
      case "get_transcript": {
        ...
        try {
          ...
          throw new McpError(
            ErrorCode.InternalError,
            `字幕処理中にエラーが発生しました: ${(error as Error).message}`
          );
        }
      }
